{ ... }:
let colors = import ~/.config/nixpkgs/colors.nix;
in with colors;
''
;;; doom-nova-theme.el --- inspired by Trevord Miller's Nova
(require 'doom-themes)

(defgroup doom-cyber-theme nil
  "Options for doom-themes"
  :group 'doom-themes)

(defcustom doom-cyber-padded-modeline doom-themes-padded-modeline
  "If non-nil, adds a 4px padding to the mode-line. Can be an integer to
determine the exact padding."
  :group 'doom-cyber-theme
  :type '(or integer boolean))

(def-doom-theme doom-cyber
  "A dark theme inspired by Kieran Blazier's Cyber."
  ...
  ((bg         '("${bg}" nil       nil))
   ...
   (fg         '("${fg}" "#c5c6c6" "white"      )) ;; TODO set correct color
   ...
   (teal       blue)
   (magenta    (doom-lighten "${magenta}" 0.3)) ; FIXME TODO set correct color
   (violet     "${dmagenta}")
   (cyan       "${dcyan}")
   ...
   (-modeline-pad
    (when doom-cyber-padded-modeline
      (if (integerp doom-cyber-padded-modeline)
          doom-cyber-padded-modeline
        4))))
...
(provide 'doom-cyber-theme)
;;; doom-cyber-theme.el ends here
''
